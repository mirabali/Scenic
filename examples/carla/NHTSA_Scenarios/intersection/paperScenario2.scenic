...
AUTHOR: Mira Bali
DESCRIPTION: Ego vehicle goes straight at 4-way intersection while second vehicle (other/adversary vehicle) on a lateral lane (not opposite lane) is making a right turn.
...
#50-50 chance of either picking the first list (lateral side) or the second list(opposite side) and then after picked, pick a specific lane there
advInitLane = Uniform(*filter(lambda m:
		m.type is ManeuverType.STRAIGHT,
		Uniform((Uniform(*filter(lambda m: 
			m.type is ManeuverType.STRAIGHT, 
			egoInitLane.maneuvers)
		).conflictingManeuvers), filter(lambda m:
		m.type is ManeuverType.STRAIGHT,
		egoManeuver.reverseManeuvers))
	)).startLane
advManeuver = Uniform(*filter(lambda m: m.type in (ManeuverType.RIGHT_TURN, ManeuverType.LEFT_TURN, ManeuverType.STRAIGHT), advInitLane.maneuvers))
